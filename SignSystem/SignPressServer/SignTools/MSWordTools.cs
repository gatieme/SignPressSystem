    ...
    public class MSWordTools : MSOfficeTools
    {
        ...
        public static void CreateHDJContractWork(string contractId)
        {
            ...
            if (!(File.Exists((String)filePath)))     // 首先检测文件是否存在
            {
                ...
                HDJContract contract = DALHDJContract.GetHDJContactAgree(contractId);       // 获取待生成的会签单信息(工作量也获取了)
        ...
        public static bool CreateHDJContractWordWithReplace(HDJContract contract, Object filePath)
        {
            ...
            wordApp = new Microsoft.Office.Interop.Word.Application();
            wordApp.Visible = false;
            //获取contemp文件夹的模版信息    通过模版id来选择
            object ConTempFileName = DEFAULT_CONTEMP_PATH + contract.ConTemp.TempId + ".doc";        //  模版信息
            //要保存在hdgcontract的文件路径
            ...
            try
            {
                //将contemp中选择的模版id号对应的模版 拷贝到hdjcontract文件夹中以会签单的编号命名
                File.Copy((string)ConTempFileName, (string)saveFileName, true);
                //打开hdgcontract文件夹的模版
                wordDoc = wordApp.Documents.Open(ref saveFileName, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing,
                    ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing);
                ...
                //月份
                string[] month = Regex.Split(contract.SubmitDate, "/", RegexOptions.IgnoreCase);
                wordApp.Selection.Find.Text = "$月份$";
                //wordApp.Selection.Find.Replacement.ClearFormatting(); 
                wordApp.Selection.Find.Replacement.Text =month[1];
                wordApp.Selection.Find.Execute(ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref replaceArea, ref Missing, ref Missing, ref Missing, ref Missing);
                //  填写表格的信息
                MSWord.Table table = wordDoc.Tables[1];
...
//                //  开始添加审核人的签名图片信息和备注信息     2016-5-5修改为12个人
//                Console.WriteLine("开始生成后8行签字栏目的信息");
...
//                }
                //#endregion
                #endregion
//通过contract.contemp.tempid 来区分不同的模版 用不同的方法

                //【1】首先填写前5行的会签单基本信息  这是几个模版通用的
                //object replaceArea = Microsoft.Office.Interop.Word.WdReplace.wdReplaceAll;
                int currTableRow = 1;
                for (currTableRow = 1; currTableRow <= 5; currTableRow++)
                {
                ...
                string _Type = contract.Id[1].ToString() + contract.Id[2].ToString();
                //【2】 分别生成不同模版签字人的信息
                #region 内河例行签字人信息
                //内河例行 内河专项
                if (_Type=="内例")
                {
                    Console.WriteLine("开始生成8个签字人的信息");
                    //插入8个人的签字照片
                    List<string> signName = new List<string>();
                    for (int signCount = 0; signCount < 8; signCount++)
                        signName.Add(DEFAULT_SIGNATURE_PATH + contract.ConTemp.SignDatas[signCount].SignEmployee.Id.ToString() + ".jpg");
                    //第六行 1、2、3
                    for (int i = 0, col = 2; i < 3; i++, col += 2)
                    {
                        object linktofile = false;
                        object savewithdocument = true;
                        object anchor = table.Cell(6, col).Range;//选中要添加图片的单元格
                        Console.WriteLine("签字人的签名图片地址" + signName[i]);
                        wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[i], ref linktofile, ref savewithdocument, ref anchor);
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 20;//图片高度
                        // 将图片设置为四周环绕型
                        MSWord.Shape s = wordDoc.Application.ActiveDocument.InlineShapes[1].ConvertToShape();
                        s.WrapFormat.Type = MSWord.WdWrapType.wdWrapSquare;
                    }
                    ...
                    //第七行  主办单位审核意见   .....
                    table.Rows[7].Height = 65;
                    replaceKey = "$" + contract.ConTemp.ColumnNames[6 - 1] + "$";
                    replaceValue = contract.ColumnDatas[6 - 1].ToString().Replace("\r", "").Replace("\n", "^p");
                    table.Range.Find.Execute(ref replaceKey, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref Missing, ref  replaceValue, ref replaceArea, ref Missing, ref Missing, ref Missing, ref Missing);
                    Console.WriteLine("替换{0}为{1}成功", replaceKey, replaceValue);

                    //第八行 1、2、3、4
                    for (int i = 4, col = 2; i < 8; i++, col += 2)
                    {
                        object linktofile = false;
                        object savewithdocument = true;
                        object anchor = table.Cell(8, col).Range;//选中要添加图片的单元格
                        Console.WriteLine("签字人的签名图片地址" + signName[i]);
                        wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[i], ref linktofile, ref savewithdocument, ref anchor);
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 20;//图片高度
                ...
                if (_Type == "界例")
                {
                    Console.WriteLine("开始生成10个签字人的信息");
                    //插入10个人的签字照片
                    List<string> signName = new List<string>();
                    for (int signCount = 0; signCount < 10; signCount++)
                        signName.Add(DEFAULT_SIGNATURE_PATH + contract.ConTemp.SignDatas[signCount].SignEmployee.Id.ToString() + ".jpg");
                    //第六行 1、2
                    for (int i = 0, col = 2; i < 2; i++, col += 2)
                    {
                        object linktofile = false;
                        object savewithdocument = true;
                        object anchor = table.Cell(6, col).Range;//选中要添加图片的单元格
                        ...
                        wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[i], ref linktofile, ref savewithdocument, ref anchor);
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 19;//图片高度
                        // 将图片设置为四周环绕型
                        MSWord.Shape s = wordDoc.Application.ActiveDocument.InlineShapes[1].ConvertToShape();
                        s.WrapFormat.Type = MSWord.WdWrapType.wdWrapSquare;
                    }
                    ...
                    object anchor1 = table.Cell(7, 4).Range;//选中要添加图片的单元格
                    Console.WriteLine("签字人的签名图片地址" + signName[2]);
                    wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[2], ref linktofile1, ref savewithdocument1, ref anchor1);
                    wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                    wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 19;//图片高度
                    // 将图片设置为四周环绕型
                    MSWord.Shape s1 = wordDoc.Application.ActiveDocument.InlineShapes[1].ConvertToShape();
                    s1.WrapFormat.Type = MSWord.WdWrapType.wdWrapSquare;


                    //第八 九 十行 4、5、6、7、8、9
                    for (int i = 3, col = 2,rowcount=8; i < 9; i++)
                    {
                        object linktofile = false;
                        object savewithdocument = true;
                        object anchor = table.Cell(rowcount, col).Range;//选中要添加图片的单元格
                        Console.WriteLine("签字人的签名图片地址" + signName[i]);
                        wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[i], ref linktofile, ref savewithdocument, ref anchor);
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 19;//图片高度
                    ...
                    object anchor2 = table.Cell(11,2).Range;//选中要添加图片的单元格
                    Console.WriteLine("签字人的签名图片地址" + signName[9]);
                    wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[9], ref linktofile2, ref savewithdocument2, ref anchor2);
                    wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                    wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 19;//图片高度
                    // 将图片设置为四周环绕型
                    MSWord.Shape s2 = wordDoc.Application.ActiveDocument.InlineShapes[1].ConvertToShape();
                    s2.WrapFormat.Type = MSWord.WdWrapType.wdWrapSquare;
                ...
                {
                    //  开始添加审核人的签名图片信息和备注信息     2016-5-5修改为12个人
                    Console.WriteLine("开始生成后12个签字人的信息");
                    //插入12个人的签字照片
                    List<string> signName = new List<string>();
                    for (int signCount = 0; signCount < 12; signCount++)
                        signName.Add(DEFAULT_SIGNATURE_PATH + contract.ConTemp.SignDatas[signCount].SignEmployee.Id.ToString() + ".jpg");
                    //第六行 1、2
                    for (int i = 0, col = 2; i < 2; i++, col += 2)
                    {
                    ...
                    //第七行 3、4、5
                    for (int i = 2, col = 2; i < 5; i++, col += 2)
                    {
                        object linktofile = false;
                        object savewithdocument = true;
                        object anchor = table.Cell(7, col).Range;//选中要添加图片的单元格
                        Console.WriteLine("签字人的签名图片地址" + signName[i]);
                        wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[i], ref linktofile, ref savewithdocument, ref anchor);
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 20;//图片高度
                    ...
                    //第八九十行 6、7、8、9、10、11 循环6下 每两下行数加1
                    int rowcount = 8;
                    int _col = 2;
                    for (int i = 5; i < 11; i++)
                    {
                        object linktofile = false;
                        object savewithdocument = true;
                        object anchor = table.Cell(rowcount, _col).Range;//选中要添加图片的单元格
                        Console.WriteLine("签字人的签名图片地址" + signName[i]);
                        wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[i], ref linktofile, ref savewithdocument, ref anchor);
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                        wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 20;//图片高度
                    ...
                    object linktofile1 = false;
                    object savewithdocument1 = true;
                    object anchor1 = table.Cell(11, 2).Range;//选中要添加图片的单元格
                    Console.WriteLine("签字人的签名图片地址" + signName[11]);
                    wordDoc.Application.ActiveDocument.InlineShapes.AddPicture((string)signName[11], ref linktofile1, ref savewithdocument1, ref anchor1);
                    wordDoc.Application.ActiveDocument.InlineShapes[1].Width = 70;//图片宽度
                    wordDoc.Application.ActiveDocument.InlineShapes[1].Height = 20;//图片高度
                    // 将图片设置为四周环绕型
                    MSWord.Shape s3 = wordDoc.Application.ActiveDocument.InlineShapes[1].ConvertToShape();
                    s3.WrapFormat.Type = MSWord.WdWrapType.wdWrapSquare;
                }
                #endregion
