...
namespace SignPressClient
{
   ...
   //在 SignPressClient.SubmitManage.SubmitManage_Load(Object sender, EventArgs e) 位置 f:\[O]GitHub\SignPress\SignPressClient\SignPressClient\SubmitManage.cs:行号 98
        ...
        private void BindSignRefuseAndAgreeOpera()
        {
            ...
            this.SignAgree.AutoGenerateColumns = false;
            DataGridViewLinkColumn recreate = new DataGridViewLinkColumn();
            recreate.Text = "重新生成会签单";
            recreate.Name = "LinkRecreate";
            recreate.HeaderText = "重新生成";
            recreate.Width = 150;
            recreate.UseColumnTextForLinkValue = true;
            this.SignAgree.Columns.Add(recreate);
        }
        ...
        private async void SignAgree_CellContentClick(object sender, DataGridViewCellEventArgs e)        //已通过列表操作列表
        {
            ...
            else if (e.ColumnIndex == 4)
            {
                ...
                {
                    //if(MessageBox.Show(, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    if (MessageBox.Show(@"该会签单是离线审批的会签单, 请问您执意要下载么？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    {
                        return;
                    }
                }
...
由此给您带来的不便，我们表示真诚的歉意", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                string Id = this.SignAgree.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (UserHelper.UserInfo.IsAdmin != 1 && UserHelper.UserInfo.CanStatistic != 1)
                {
                    MessageBox.Show(@"您没有权限重新生成会签单", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (HDJContract.GetIsOnlineFromContractId(Id) == false)
                {
                    //MessageBox.Show(@"该会签单是离线审批的会签单, 无法通过本系统进行生成", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    if (MessageBox.Show(@"该会签单是离线审批的会签单, 请问您执意要重新生成么？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    {
                        return;
                    }
                }

                MessageBox.Show(@"重新生成会签单以后请不要立即在系统中下载, 否则会导致系统崩溃, 因为服务器可能正在生成你的请求，生成的过程需要几分钟", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this._sc.ReCreateHDJContract(Id);
            }
        }
        ...
        private void b_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                for (int i = num + 1; i <= total; i++)
                {
                    ...
                    TextBox workInfo1 = (TextBox)this.ProjectPanel.Controls["WorkInfo_" + i.ToString()];
                    ...
                    workInfo1.Location = new Point(workInfo1.Location.X, workInfo1.Location.Y - 34);
                    ...
                    workInfo1.Name = "WorkInfo_" + (i - 1).ToString();
