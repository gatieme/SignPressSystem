...
**实现思路**
在服务器接收到客户的提交会签单请求后, 直接将会签单的totalresult置为1
然后服务器生成会签单的信息, 同时生成统计信息

**具体实现**
在服务器处理逻辑RaiseInsertHDJContractRequest中添加
当插入会签单成功后
*	判断如果会签单是离线的会签单GetIsOnlineFromContractId
*	处理离线会签单将totalresult置为1， DealOfflineHDJContract
*	然后检查离线会签单totalresult是否设置成功 IsContractAgree
*	接着增加生成会签单的请求AddContractWork
*	接着增加统计会签单的请求AddStatisticWork

**bug**

因为需求要求离线会签单也得生成, 那么就需要增加离线会签单的生成的接口
AddContractWork的处理函数CreateHDJContractWork中生成会签单的时候需要
先查询会签单的信息
但是之前的接口GetHDJContactAgree是针对在线的单子的
离线的单子没有经过人签字，没有签字人的result信息和remark信息
因此需要增加新的接口完成此工作, 即GetHDJContactOffline

并在AddContractWork的处理函数CreateHDJContractWork中获取会签单时对在线和离线进行判断, 并调用不同的接口

```c
HDJContract contract = null;
if (DALHDJContractWithWorkload.GetIsOnlineFromContractId(contractId) == true)
{
    contract = DALHDJContract.GetHDJContactAgree(contractId);       // 获取待生成的会签单信息
}
else
{
    contract = DALHDJContract.GetHDJContactOffline(contractId);       // 获取待生成的会签单信息
}
```language
```
