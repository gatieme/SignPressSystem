...
//调用的这个来下载会签单信息
        ...
        private void DownloadContractFile(Socket socketClient, String contractId)
        {
            ...
            if(!(File.Exists((String)filePath)))     // 首先检测文件是否存在
            {
                ...
                HDJContract contract = DALHDJContract.GetHDJContactAgree(contractId);       // 获取待生成的会签单信息（全部信息）
                Console.WriteLine("获取待生成的会签单信息成功");
                //将获取到的会签单信息插入到会签单word模版中
            ...
            //设置数据库signaturestatus 中的isdownload 为1
