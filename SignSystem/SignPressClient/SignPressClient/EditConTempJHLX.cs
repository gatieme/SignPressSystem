...
{
    public partial class EditConTempJHLX : Form
    {
        int Id;
        SignSocketClient _sc;
        public EditConTempJHLX()
        {
            InitializeComponent();
        }
        public EditConTempJHLX(int ConTempId, SignSocketClient sc)
            : this()
        {
            Id = ConTempId;
            _sc = sc;
        ...
        #region 数据加载
        private void EditConTempJHLX_Load(object sender, EventArgs e)
        {
            BindDepartment(UserHelper.DepList);               //绑定部门信息
            try
            {
                Templete temp = new Templete();
                temp = _sc.GetContractTemplateJHLX(Id);
