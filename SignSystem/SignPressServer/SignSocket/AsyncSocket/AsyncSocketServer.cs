...
namespace SignPressServer.SignSocket.AsyncSocket
{
    ...
    public class AsyncSocketServer: IDisposable
    {
        ...
        #endregion
//在构造函数中启动异步tcp  文件传输服务（端口是6060）  日志服务
        #region 构造函数
...
/// 2016-4-18    增加内河会签单模版  INSERT_CONTRACT_TEMPLATE_INSIDE_REQUEST
                    #region  会签单模版操作
                    /// <summary>
                    /// ==会签单模版操作==
                    ...
                    /// 2016-4-18    增加内河会签单模版  INSERT_CONTRACT_TEMPLATE_INSIDE_REQUEST
                    ///              查询会签单模版     QUERY_CONTRACT_TEMPLATE_INSIDE_REQUEST
                    ///2016-4-20              
                    ///                查询不同类型会签单模版     QUERY_CONTRACT_TEMPLATE_ALL_REQUEST
                    /// </summary>
...
//没有用了
                    ...
                    case "GET_CONTRACT_TEMPLATE_REQUEST":
                        ...
                        break;
//2016-4-18    增加内河会签单模版请求出来
                    case "INSERT_CONTRACT_TEMPLATE_INSIDE_REQUEST":
                        RaiseInsertContractTemplateInsideRequest(state);
                        break;
//在contemp 中查询所有模版的 id 模版名称 模版代码 创建时间    
                    case "QUERY_CONTRACT_TEMPLATE_INSIDE_REQUEST":
                        RaiseQueryContractTemplateInsideRequest(state);
                        break;
//2016-4-20
                    case "GET_CONTRACT_TEMPLATE_INSIDE_REQUEST":
                        RaiseGetContractTemplateInsideRequest(state);
                        break;
//没有用了
                    case "QUERY_CONTRACT_TEMPLATE_JHLX_REQUEST":
                        RaiseQueryContractTemplateJHLXRequest(state);
                        break;

                    case "GET_CONTRACT_TEMPLATE_JHLX_REQUEST":
                        RaiseGetContractTemplateJHLXRequest(state);
                        break;
                    #endregion
                    ...
                    case "STATISTIC_DEP_YEAR_PRO_REQUEST":
                        ...
                        break;
                    //  需求更改, 2016-04-05 21：10
                    //   累计申请额度，返回的是当前部门当年category的当前申请总额
                    case "STATISTIC_DEP_YEAR_CATEGORY_REQUEST":
                        RaiseStatisticDepartmentYearCategoryRequest(state);
                        break;
                    #endregion
        ...
        private void RaiseInsertContractTemplateRequest(AsyncSocketState state)
        {
            ...
            else
            {
                ...
                response = ServerResponse.INSERT_CONTRACT_TEMPLATE_FAILED;                //  插入失败信号
            }
...
//2016-4-18增加处理客服端的插入内河会签单模版的请求
        #region 处理客户端的插入内河会签单模版的请求
        /// <summary>
        /// 处理客户端的插入内河会签单模版的请求
        /// </summary>
        /// <param name="state"></param>
        private void RaiseInsertContractTemplateInsideRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的待插入内河会签单模版" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "待插入内河会签单模版" + state.SocketMessage.Message, LogMessageType.Information));

            ServerResponse response = new ServerResponse();

            // json数据解包
            ContractTemplateInside conTemp = JsonConvert.DeserializeObject<ContractTemplateInside>(state.SocketMessage.Message);
            //数据访问层处理插入内河模版操作
            bool result = DALContractTemplate.InsertContractTemplate_INSIDE(conTemp);

            if (result == true)
            {
                Console.WriteLine("内河会签单模版{0}插入成功", conTemp.Name);
                this.Log.Write(new LogMessage("内河会签单模版" + conTemp.Name + "插入成功", LogMessageType.Success));

                //  插入成功信号
                response = ServerResponse.INSERT_CONTRACT_TEMPLATE_INSIDE_SUCCESS;

            }
            else
            {
                Console.WriteLine("内河会签单模版{0}插入失败", conTemp.Name);
                this.Log.Write(new LogMessage("内河会签单模版" + conTemp.Name + "插入失败", LogMessageType.Error));

                response = ServerResponse.INSERT_CONTRACT_TEMPLATE_INSIDE_FAILED;                //  插入失败信号
            }
...
//2016-04-18增加查询内河模版请求
//2016-4-23修改为查询所以模版信息  在contemp表中
        #region 查询内河会签单模版的信息请求
        /// <summary>
        /// 处理客户端的查询内河会签单模版请求
        /// </summary>
        /// <param name="state"></param>
        private void RaiseQueryContractTemplateInsideRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的查询会签单模版信息" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "查询会签单模版信息" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            List<ContractTemplate> conTemps = DALContractTemplate.QueryContractTemplateInside();
            this.Log.Write(new LogMessage(state.ClientIp + "查询到的了所有的会签单", LogMessageType.Information));
            ...
            if (conTemps != null)
            {
                ...
                //QUERY_EMPLOYEE_RESPONSE = "QUERY_EMPLOYEE_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.QUERY_CONTRACT_TEMPLATE_INSIDE_SUCCESS;
            }
            else
            {
                Console.WriteLine("会签单模版信息查询失败");
                ...
                //QUERY_EMPLOYEE_RESPONSE = "QUERY_EMPLOYEE_FAILED";                //  用户登录失败信号
                response = ServerResponse.QUERY_CONTRACT_TEMPLATE_INSIDE_FAILED;
            }

            //  查询会签单成功则同时发送[报头 + 部门信息] 
            if (response.Equals(ServerResponse.QUERY_CONTRACT_TEMPLATE_INSIDE_SUCCESS))
            {
                AsyncSocketMessage socketMessage = new AsyncSocketMessage(response, conTemps);
                this.Send(state.ClientSocket, Encoding.UTF8.GetBytes(socketMessage.Package));                    //  将
            }
...
//2016-04-20增加查询不同模版信息请求
        #region 查询不同模版会签单模版的信息请求
        /// <summary>
        /// 处理客户端的查询不同模版会签单模版请求
        /// </summary>
        /// <param name="state"></param>
        private void RaiseQueryContractTemplateJHLXRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的查询所有会签单模版信息" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "查询所有会签单模版信息" + state.SocketMessage.Message, LogMessageType.Information));

            ServerResponse response = new ServerResponse();

            List<ContractTemplate> conTemps = DALContractTemplate.QueryContractTemplateJHLX();
            this.Log.Write(new LogMessage(state.ClientIp + "查询到的了所有的不同类型会签单", LogMessageType.Information));

            if (conTemps != null)
            {
                Console.WriteLine("所有不同类型会签单模版信息查询成功");
                this.Log.Write(new LogMessage(state.ClientIp + "所有不同类型会签单模版信息信息查询成功", LogMessageType.Success));

                //QUERY_EMPLOYEE_RESPONSE = "QUERY_EMPLOYEE_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.QUERY_CONTRACT_TEMPLATE_ALL_SUCCESS;
            }
            else
            {
                Console.WriteLine("所有不同类型会签单模版信息查询失败");
                this.Log.Write(new LogMessage(state.ClientIp + "所有不同类型会签单模版信息查询失败", LogMessageType.Error));

                //QUERY_EMPLOYEE_RESPONSE = "QUERY_EMPLOYEE_FAILED";                //  用户登录失败信号
                response = ServerResponse.QUERY_CONTRACT_TEMPLATE_ALL_FAILED;
            }

            //  查询会签单成功则同时发送[报头 + 部门信息] 
            if (response.Equals(ServerResponse.QUERY_CONTRACT_TEMPLATE_ALL_SUCCESS))
            {
                AsyncSocketMessage socketMessage = new AsyncSocketMessage(response, conTemps);
                this.Send(state.ClientSocket, Encoding.UTF8.GetBytes(socketMessage.Package));                    //  将
            }
        ...
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的待获取界河专项会签单模版编号" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "待获取界河专项会签单模版编号" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            if (conTemp != null)
            {
                Console.WriteLine(state.ClientIp + "获取界河专项会签单模版{0}成功", conTempId);
                this.Log.Write(new LogMessage(state.ClientIp + "获取界河专项会签单模版" + conTempId + "成功", LogMessageType.Success));
            ...
            {
                Console.WriteLine(state.ClientIp + "获取界河专项会签单模版{0}失败", conTempId.ToString());
                this.Log.Write(new LogMessage(state.ClientIp + "获取界河专项会签单模版" + conTempId + "失败", LogMessageType.Error));
...
//2016-4-20
//2016-4-24  修改为可以获取全部模版信息   通过id  type
        #region  获取内河会签单模版的信息
        /// <summary>
        /// 获取内河会签单模版的具体信息
        /// </summary>
        /// <param name="state"></param>
        private void RaiseGetContractTemplateInsideRequest(AsyncSocketState state)
        {
            ...
            // json数据解包
            int conTempId = JsonConvert.DeserializeObject<int>(state.SocketMessage.Message);
            ContractTemplate conTemp = DALContractTemplate.GetContractTemplateInside(conTempId);
            ...
            if (conTemp != null)
            {
                ...
                //MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.GET_CONTRACT_TEMPLATE_INSIDE_SUCCESS;
            }
            else
            {
                ...
                // MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_FAILED";                //  用户登录失败信号
                response = ServerResponse.GET_CONTRACT_TEMPLATE_INSIDE_FAILED;
            }

            //  查询会签单成功则同时发送[报头 + 部门信息] 
            if (response.Equals(ServerResponse.GET_CONTRACT_TEMPLATE_INSIDE_SUCCESS))
            {
                AsyncSocketMessage socketMessage = new AsyncSocketMessage(response, conTemp);
                this.Send(state.ClientSocket, Encoding.UTF8.GetBytes(socketMessage.Package));                    //  将
            }
        ...
        #endregion
//2016-4-29 界河例行
        #region  获取界河例行会签单模版的信息
        /// <summary>
        /// 获取内河会签单模版的具体信息
        /// </summary>
        /// <param name="state"></param>
        private void RaiseGetContractTemplateJHLXRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的待获取会签单模版编号" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "待获取会签单模版编号" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            // json数据解包
            int conTempId = JsonConvert.DeserializeObject<int>(state.SocketMessage.Message);
            ContractTemplate conTemp = DALContractTemplate.GetContractTemplateJHLX(conTempId);
            ...
            if (conTemp != null)
            {
                ...
                //MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.GET_CONTRACT_TEMPLATE_JHLX_SUCCESS;
            }
            else
            {
                Console.WriteLine(state.ClientIp + "获取会签单模版{0}失败", conTempId.ToString());
                ...
                // MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_FAILED";                //  用户登录失败信号
                response = ServerResponse.GET_CONTRACT_TEMPLATE_JHLX_FAILED;
            }

            //  查询会签单成功则同时发送[报头 + 部门信息] 
            if (response.Equals(ServerResponse.GET_CONTRACT_TEMPLATE_JHLX_SUCCESS))
            {
                AsyncSocketMessage socketMessage = new AsyncSocketMessage(response, conTemp);
                this.Send(state.ClientSocket, Encoding.UTF8.GetBytes(socketMessage.Package));                    //  将
            }
        ...
        private void RaiseInsertSignDetailRequest(AsyncSocketState state)
        {
            ...
            ///  那么该会签单文档就可以被生成了    
            ///  生成的会签单信息存在服务器端   等下次用户下载的时候就不用再去重新生成了，直接把生成好了的发送给用户就行了
            if (detail.Result == 1)                  //  如果签字同意
            {
                if (DALSignatureDetail.IsContractAgree(detail.ConId) == true)   // 如果这张单子已经被同意了
                {
                    ...
                    //2016-5-6  先把统计部分注释掉   
                    // 同时统计的信息也可以被生成  首先先得到年份 工程简称  工程的id
        ...
        #region 用于客户端实时统计信息的返回
        /// <summary>
        /// 用于会签单中的累计申请额度, 实际返回的是当年当前部门申请的小项目project的总金额
        /// </summary>
        /// <param name="state"></param>
        private void RaiseStatisticDepartmentYearProjectRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自{0}的待查询部门当前工程的统计信息{1}", state.ClientIp, state.SocketMessage.Message); // 输出真正的信息
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "的待查询部门当前工程的统计信息" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            if (workload != null)
            {
                Console.WriteLine("查询部门当前工程的统计信息成功");
                this.Log.Write(new LogMessage(state.ClientIp + "查询部门当前工程的统计信息成功", LogMessageType.Success));
            ...
            {
                Console.WriteLine("查询部门当前工程的统计信息失败");
                this.Log.Write(new LogMessage(state.ClientIp + "查询部门当前工程的统计信息失败", LogMessageType.Error));
        ...
        /// <summary>
        /// 用于会签单中的累计申请额度, 实际返回的是当年当前部门申请的类别category的总金额
        /// </summary>
        /// <param name="state"></param>
        private void RaiseStatisticDepartmentYearCategoryRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自{0}的待查询部门当前工程的统计信息{1}", state.ClientIp, state.SocketMessage.Message); // 输出真正的信息
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "的待查询部门当前工程的统计信息" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            Search search = JsonConvert.DeserializeObject<Search>(state.SocketMessage.Message);
            // 向数据库中查询部门的信息
            ContractWorkload workload = DALContractStatistic.StatisDepartmentYearCategoryWorkLoad(search.SDepartmentShortlCall, search.CategoryShortCall, search.Year);
            ...
            if (workload != null)
            {
                ...
                //QUERY_DEPARTMENT_RESPONSE = "QUERY_DEPARTMENT_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.STATISTIC_DEP_YEAR_CATEGORY_SUCCESS;
            ...
            else
            {
                ...
                //QUERY_DEPARTMENT_RESPONSE = "QUERY_DEPARTMENT_FAILED";                //  用户登录失败信号
                response = ServerResponse.STATISTIC_DEP_YEAR_CATEGORY_FAILED;
            }

            //  查询部门成功则同时发送[报头 + 部门信息] 
            if (response.Equals(ServerResponse.STATISTIC_DEP_YEAR_CATEGORY_SUCCESS))
            {
                AsyncSocketMessage socketMessage = new AsyncSocketMessage(response, workload);
                this.Send(state.ClientSocket, Encoding.UTF8.GetBytes(socketMessage.Package));                    //  将
            }
