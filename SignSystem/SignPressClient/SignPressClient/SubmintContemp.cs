...
{
    public partial class SubmintContemp : Form
    {
        SignSocketClient _sc;
        int ContempId;
        Templete temp = new Templete();
        public SubmintContemp()
        {
            InitializeComponent();
        }

        public SubmintContemp(int contempId,SignSocketClient sc):this()
        {
            _sc = sc;
            //模版id
            ContempId = contempId;
             try
            {
               // Templete temp = new Templete();
                //SOCKET查询
                temp = _sc.GetContractTemplate(ContempId);
                ...
                this.SignPer11.Text = temp.SignDatas[10].SignEmployee.Name.ToString();
                this.SignPer12.Text = temp.SignDatas[11].SignEmployee.Name.ToString();
            }
            catch
            {
        ...
        private void SubmintContemp_Load(object sender, EventArgs e)
        {
        ...
        private void BindCategory(bool isFlush)
        {
            ...
            if (UserHelper.ContractCategoryList == null || isFlush == true)
            {
                ...
                this.IdCategory.SelectedIndex = 1;
               //栏目一的工程名
        ...
        #region 提交事件
        //提交按钮
        private void btnSubmit_Click(object sender, EventArgs e)                   //提交会签单信息
        {
            //全部空不为空
