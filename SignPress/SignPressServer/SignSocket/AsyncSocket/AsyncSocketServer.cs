    ...
    public class AsyncSocketServer: IDisposable
    {
        ...
        public void DealClientRequest(AsyncSocketState state)
        {
            ...
            do
            {
                ...
                switch (state.SocketMessage.Head)
                {
                    ...
                    case "CREATE_HDJCONTRACT_REQUEST":        //  插入任务工作量信息
                        RaiseCreateHDJContractRequest(state);
                        break;
                    case "RECREATE_HDJCONTRACT_REQUEST":        //  插入任务工作量信息
                        RaiseReCreateHDJContractRequest(state);
                        break;             
                    #endregion
        ...
        private void RaiseInsertHDJContractRequest(AsyncSocketState state)
        {
            ...
            else
            {
                ...
                //  modify by gatieme for 离线会签单的处理
                if (result == true)
                {
                    ...
                    //  如果是离线的单子,
                    //  首先处理离线单子，直接通过         DealOfflineHDJContract设置totlalResult = 1;
                    //  然后检查单子是否已经被设置为通过    IsContractAgree
                    //  接着增加生成会签单的信息           AddContractWork
                    //  最后添加生成统计信息              AddStatisticWork
                    if (DALHDJContract.GetIsOnlineFromContractId(contract.Id) == false)
                    {
                        if (DALHDJContractWithWorkload.DealOfflineHDJContract(contract.Id) == true)
                        { 
                            #region  检查是否通过, 添加生成会签单信息, 添加生成统计信息
                            if (DALSignatureDetail.IsContractAgree(contract.Id) == true)   // 如果这张单子已经被同意了
                            {
                                this.m_msofficeServer.AddContractWork(contract.Id);

                                // 重新生成会签单的时候同时统计的信息也可以被生成
                                int year = DALHDJContract.GetYearFromContractId(contract.Id);
                                string categoryShortCall = DALHDJContract.GetCatgoryShortCallFromContractId(contract.Id).ToString();
                                ...
                                Console.WriteLine(state.ClientIp + "生成会签单{0}信息已经入队，请等待", contract.Id);
                                this.Log.Write(new LogMessage(state.ClientIp + "生成会签单" + contract.Id + "信息已经入队，请等待", LogMessageType.Success));
                            }
                            else
                            {
                                Console.WriteLine(state.ClientIp + "离线会签单{0}检查通过失败", contract.Id);
                                this.Log.Write(new LogMessage(state.ClientIp + "离线会签单" + contract.Id + "检查通过失败", LogMessageType.Success));
                            }
                            #endregion
                        }
                        else
                        {
                            Console.WriteLine(state.ClientIp + "离线会签单{0}设置通过失败", contract.Id);
                            this.Log.Write(new LogMessage(state.ClientIp + "离线会签单" + contract.Id + "设置通过失败", LogMessageType.Success));
                        }
                    }
                    //  用户登录成功信号
        ...
        /// </summary>
        /// <param name="state"></param>
        private void RaiseCreateHDJContractRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的待生成的会签单编号" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "的待生成的会签单编号" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            if (DALSignatureDetail.IsContractAgree(contractId) == true)   // 如果这张单子已经被同意了
            {
                this.m_msofficeServer.AddContractWork(contractId);

                // 重新生成会签单的时候同时统计的信息也可以被生成
                int year = DALHDJContract.GetYearFromContractId(contractId);
                string categoryShortCall = DALHDJContract.GetCatgoryShortCallFromContractId(contractId).ToString();
                int categoryId = DALContractIdCategory.GetCategory(categoryShortCall).Id;
                this.m_msofficeServer.AddStatisticWork(year, categoryId);

                Console.WriteLine(state.ClientIp + "生成会签单{0}信息已经入队，请等待", contractId);
                this.Log.Write(new LogMessage(state.ClientIp + "生成会签单" + contractId + "信息已经入队，请等待", LogMessageType.Success));

                //MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.CREATE_HDJCONTRACT_SUCCESS;
            }
            else
            {
                Console.WriteLine(state.ClientIp + "获取会签单{0}失败", contractId.ToString());
                ...
                // MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_FAILED";                //  用户登录失败信号
                response = ServerResponse.CREATE_HDJCONTRACT_FAILED;
            }
        ...
        private void RaiseReCreateHDJContractRequest(AsyncSocketState state)
        {
            Console.WriteLine("接收到来自" + state.ClientIp + "的待重新生成的会签单编号" + state.SocketMessage.Message);
            this.Log.Write(new LogMessage("接收到来自" + state.ClientIp + "的待重新生成的会签单编号" + state.SocketMessage.Message, LogMessageType.Information));
            ...
            if (DALSignatureDetail.IsContractAgree(contractId) == true)   // 如果这张单子已经被同意了
            {
                ...
                //if (DALHDJContract.GetIsOnlineFromContractId(contractId) == false)
                //{
                //    重新生成会签单的时候同时统计的信息也可以被生成
                //    int year = DALHDJContract.GetYearFromContractId(contractId);
                //    string categoryShortCall = DALHDJContract.GetCatgoryShortCallFromContractId(contractId).ToString();
                ...
                //    this.m_msofficeServer.AddStatisticWork(year, categoryId);
                //}
                Console.WriteLine(state.ClientIp + "重新生成会签单{0}信息已经入队，请等待", contractId);
                this.Log.Write(new LogMessage(state.ClientIp + "重新生成会签单" + contractId + "信息已经入队，请等待", LogMessageType.Success));

                //MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_SUCCESS";               //  用户登录成功信号   
                response = ServerResponse.RECREATE_HDJCONTRACT_SUCCESS;
            }
            else
            {
                Console.WriteLine(state.ClientIp + "重新生成会签单{0}失败", contractId.ToString());
                this.Log.Write(new LogMessage(state.ClientIp + "重新生成会签单" + contractId + "失败", LogMessageType.Error));

                // MODIFY_EMPLOYEE_RESPONSE = "MODIFY_EMPLOYEE_FAILED";                //  用户登录失败信号
                response = ServerResponse.RECREATE_HDJCONTRACT_FAILED;
            }
