...
//2016-4-18增加添加内河模版功能
//2016-4-23  统一修改为操作一个contemp表   这个函数都没用了
        /// <summary>
        /// 添加内河模板
        /// </summary>
        /// <param name="temp"></param>
        /// <returns></returns>
        public string AddConTemplete_Inside(Templete temp)
        {
            //return Task.Factory.StartNew(() =>
            {
                try
                {
                    //scoket发送添加内河模块请求信息
                    SocketMessage sm = new SocketMessage(Request.INSERT_CONTRACT_TEMPLATE_INSIDE_REQUEST, temp);
                    ClientSocket.Send(Encoding.UTF8.GetBytes(sm.Package));
                    ...
                    if (Msg[0] == Response.INSERT_CONTRACT_TEMPLATE_INSIDE_SUCCESS.ToString())
                    {
                        Logging.AddLog("添加内河模板成功!");
                    }
                    else
                    {
                        Logging.AddLog("添加内河模版失败!");
                    }
                    return Msg[0];
                }
                catch
                {
                    Logging.AddLog("添加内河模板失败（服务器连接中断）!");
                    return "添加内河模板";
                }
                // });
            }
...
//2016-04-18
//2016-4-21修改为查询全部模版信息
        /******************************************************************************************
         * 2016-4-21  用原来的Inside 代替 ALL 增加一个查询不同类型模版信息的方法，让我能得到现在所有不同模版的信息，好让可以一起显示在列表中*
         * 2014-4-23  将所有信息全部写到contemp这个表中 可以存放不同类型的模版  同过type字段来区分模版类型
         ******************************************************************************************/
        /// <summary>
        /// 查询所有模板信息
        /// </summary>
        /// <returns></returns>
        public async Task<List<Templete>> QueryContractTemplateInside()
        {
            return await Task.Factory.StartNew(() =>
            {
                try
                {
                    SocketMessage sm = new SocketMessage(Request.QUERY_CONTRACT_TEMPLATE_INSIDE_REQUEST);
                    ClientSocket.Send(Encoding.UTF8.GetBytes(sm.Package));
                    ...
                    if (Msg[0] == Response.QUERY_CONTRACT_TEMPLATE_INSIDE_SUCCESS.ToString())
                    {
                        List<Templete> list = new List<Templete>();
                        list = JsonConvert.DeserializeObject<List<Templete>>(Msg[2]);
                    ...
                    {
                        Logging.AddLog("查询全部模板列表失败!");
                        return null;
                    }
                }
                catch
                {
                    Logging.AddLog("查询全部模板列表失败(服务器连接中断)!");
                    return null;
                }
        ...
        public Templete GetContractTemplate(int tempId)
        {
            //return Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    {
                        Logging.AddLog("查询界河专项模板列表失败!");
                        return null;
                    }
                }
                catch
                {
                    Logging.AddLog("查询界河专项模板列表失败(服务器连接中断)!");
                    return null;
                }
            }//);
        }
//2016-4-20  增加查询待定内河模版信息
        //2016-4-23   修改为全部放在Templete中     
        /// <summary>
        /// 查询内河特定模板信息
        /// </summary>
        /// <param name="tempId"></param>
        /// <returns></returns>
        public Templete GetContractTemplateInside(int tempId)
        {
            //return Task.Factory.StartNew(() =>
            {
                try
                {
                    SocketMessage sm = new SocketMessage(Request.GET_CONTRACT_TEMPLATE_INSIDE_REQUEST, tempId);
                    ClientSocket.Send(Encoding.UTF8.GetBytes(sm.Package));
                    ...
                    if (Msg[0] == Response.GET_CONTRACT_TEMPLATE_INSIDE_SUCCESS.ToString())
                    {
                        Templete list = new Templete();
                        list = JsonConvert.DeserializeObject<Templete>(Msg[2]);
                    ...
                    {
                        Logging.AddLog("查询特定内河模板列表失败!");
                        return null;
                    }
                }
                catch
                {
                    Logging.AddLog("查询特定内河模板列表失败(服务器连接中断)!");
                    return null;
                }
            }//);
        }

//2016-4-29增加查询界河例行模版信息
        public Templete GetContractTemplateJHLX(int tempId)
        {
            //return Task.Factory.StartNew(() =>
            {
                try
                {
                    SocketMessage sm = new SocketMessage(Request.GET_CONTRACT_TEMPLATE_JHLX_REQUEST, tempId);
                    ClientSocket.Send(Encoding.UTF8.GetBytes(sm.Package));
                    ...
                    if (Msg[0] == Response.GET_CONTRACT_TEMPLATE_JHLX_SUCCESS.ToString())
                    {
                        Templete list = new Templete();
                        list = JsonConvert.DeserializeObject<Templete>(Msg[2]);
                    ...
                    {
                        Logging.AddLog("查询特定界河例行模板列表失败!");
                        return null;
                    }
                }
                catch
                {
                    Logging.AddLog("查询特定界河例行模板列表失败(服务器连接中断)!");
                    return null;
                }
            }//);
        }
        ...
        public string ModifyContractTemplate(Templete temp)
        {
            //return Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    if (Msg[0] == Response.MODIFY_CONTRACT_TEMPLATE_SUCCESS.ToString())
                    {
        ...
        /// 下载会签单信息
        /// </summary>
        /// <param name="Id">会签单编号</param>
        /// <param name="fileepath">路径</param>
        /// <returns></returns>
