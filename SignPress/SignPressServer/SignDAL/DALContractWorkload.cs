...
namespace SignPressServer.SignDAL
{
    public class DALContractWorkload
    {
        ...
        private const String QUERY_CONTRACT_WORKLOAD_STR = @"SELECT w.contractid contractid, i.id itemid, i.item item, i.projectid projectid, w.work work, w.workinfo workinfo, w.expense expense FROM `workload` w, `item` i WHERE (w.itemid = i.id and `contractid` = @ContractId)";
        ...
        /// <returns></returns>
        private static String INSERT_WORKLOAD_STR = @"INSERT INTO `workload` (`id`, `contractid`, `itemid`, `work`, `workinfo`, `expense`) VALUES (@Id, @ContractId, @ItemId, @Work, @WorkInfo, @Expense)";
        public static bool InsertWorkload(ContractWorkload workload)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@WorkInfo", workload.WorkInfo);                 //  当前工作量的工作量单位或者备注信息
        ...
        #region    修改工作量信息
        private const String MODIFY_WORKLOAD_STR = @"UPDATE `workload` SET `work` = @Work, `workinfo` = @WorkInfo`, expense` = @Expense WHERE (`contractid` = @ContractId and`itemid` = @ItemId)";
        /// <summary>
        /// 修改工作量信息
        /// </summary>
