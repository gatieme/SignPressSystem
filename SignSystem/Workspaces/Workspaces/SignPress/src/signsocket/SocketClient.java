...
public class SocketClient
{
   ...
   // private static final String SERVER_IP   = "218.7.0.37"; //"221.212.58.67";
    private static final String SERVER_IP   = "222.171.195.97"; //"221.212.58.67";
    //private static final String SERVER_IP   =  "10.0.51.141";
    private static final int    SERVER_PORT      = 8888;//7777;
	...
	public ArrayList<SHDJContract> QueryUnsignedHDJContract(int employeeId)
	{
		...
		try
		{
			...
			m_recvBuffer = new byte[1024 * 4];
			in.read(m_recvBuffer, 0, 1024 * 4);
			//int recvlen = in.read(m_recvBuffer, 0, 1024 * 4);
	...
	public ArrayList<SHDJContract> QuerySignedTopHDJContract(int employeeId)
	{
		ArrayList<SHDJContract> contracts = new ArrayList<SHDJContract>();
	    Type type = new TypeToken<ArrayList<SHDJContract>>(){}.getType(); 
		try
		{
			//  发送查询为签字的会签单的信息以及员工的ID
			SocketMessage message = new SocketMessage(ClientRequest.QUERY_SIGNED_CONTRACT_TOP_REQUEST, employeeId);
			...
			if(message.Head.equals(ServerResponse.QUERY_SIGNED_CONTRACT_TOP_SUCCESS.toString()))
			{			
				//  数据头是查询成功
				Gson gson = new Gson();
				contracts = gson.fromJson(message.Message, type);				
