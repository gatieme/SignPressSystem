...
namespace SignPressClient
{
    public partial class MainWindow : Form
    {
        ...
        public MainWindow()
        {
            ...
            if (emp.CanSign == 1)            //判断是否有签字和提交的权限   有的话开Timer进行计时查询
            {
        ...
        #region 两个Timer定时刷新数据
        private void Submit_timer_Elapsed(object sender, ElapsedEventArgs e)
        {
            //1S =  1000ms  
            this.Submit_timer.Interval = 1800*1000;  //1800 000ms = 1800s = 30min
...
//                List<SHDJContract> agreeList = _sc.QuerySignAgreeUndownload(UserHelper.UserInfo.Id);
        ...
        {
            this.UnSign_timer.Interval = 1200*1000;         //  设置1min = 60s = 60000ms刷新一次  这里是20min刷新一次
            this.UnSign_timer.Start();
            BindSignData();  
        }

        //绑定签字管理的内容
        public  void BindSignData()                              //绑定待办列表的个数
        {
            try
            {
